# Julia Kenny / Xiaowei Jia 1656 / Project 1
...
class Bike():
    ...
    def total_bikes(self):
        ...
        bikeTotal = self.df2['num_bikes_available'].sum()
        return bikeTotal

    def total_docks(self):
        # return the total number of docks available
        dockTotal = self.df2['num_docks_available'].sum()
        return dockTotal
    ...
    def percent_avail(self, station_id):
        ...
        if str(station_id) in self.df2.values:
            ...
            numbikes = row.num_bikes_available.item()
            numdocks = row.num_docks_available.item()
            return str(int((numdocks/(numbikes + numdocks))*100))+"%"
        else:
            return ''
    ...
    def closest_stations(self, latitude, longitude):
        ...
        for index, row in self.merge.iterrows():
            ...
            self.df3 = self.df3.append({'dist':dist, 'station_id':row['station_id'], 'name': row['name']},ignore_index = True)
